...
const parseTree = async props => {
  ...
  if (path) {
    if (!tree[type][path]) {
      ...
      tree.cnt--
      if (!tree.cnt) {
        filterModules(tree, store, props.treeshake)
        tree.done()
      }
    }
  }
  ...
  if (!props.type) {
    ...
    if (server) {
      ...
      parseTree({
        ...
        type: 'node',
        tree,
        treeshake: props.treeshake
      })
    } else {
      parseTree({ ...props, type: 'node', tree, treeshake: props.treeshake })
    }
    parseTree({ ...props, type: 'browser', tree, treeshake: props.treeshake })
  } else {
    parseTree({ ...props, tree, treeshake: props.treeshake })
  }
}
