...
const generateCode = async props => {
  ...
  if (!bundle.requiredInBundle) {
    ...
    if (type === 'node') {
      ...
      if (bundle.env.size > 0) {
        const obj = { env: {} }
        bundle.env.forEach(v => {
          obj.env[v] = process.env[v] || ''
        })

        bundle.envFile = obj
        // bundle.code = processEnv + bundle.code
        // bundle.es2015 = processEnv + bundle.es2015
      }
