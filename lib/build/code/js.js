...
const mapAllExports = (leveledCJS, code, parsed, target, field) => {
  ...
  const p = `\n// ALL parse -${target.id} \n\n function ${
    ...
    .map(
      name =>
        ` if (${field}.${name} === void 0) { ${field}.${name} = ${target.id}_${name}; }\n`
    )
    .join('')} }\n\n`
...
const makeMember = (id, member) => {
  return `global.${id}_${member} = (typeof ${id}_${member} === 'undefined' ? undefined :  ${id}_${member});`
}

const addToGlobal = (file, js) => {
  const id = file.id
  const shakeIt = file.browser.parsed.treeShakedMembers
  if (shakeIt && !shakeIt.static) {
    js.push(`global.${id} = ${id}`)

    if (shakeIt.all) {
      file.browser.parsed.exportStats.export.forEach(member => {
        js.push(makeMember(id, member))
      })
      file.browser.parsed.exportStats.moduleExports.forEach(member => {
        js.push(makeMember(id, member))
      })
    } else {
      // need to see if its a require thing
      for (const member in shakeIt.members) {
        js.push(makeMember(id, member))
      }
    }
  } else {
...
const parseJs = props => {
  ...
  // if (file.resolved.path.indexOf('react-native-web/dist/index.js') !== -1) {
  //   // $11nwfyq_View
  //   // rewriteModule()

  //   // and add some stuff
  //   code = code.replace(/var \$11nwfyq_View = .+;/, `var $11nwfyq_View ='div'`)

  //   // code = code.replace(/var \$11nwfyq_Text = .+;/, `var $11nwfyq_Text ='div'`)
  // }
    ...
    ({
      ...
      if (restoreModule) {
        if (type === 'require') {
          ...
          const safe = `(() => {var r={};try{ r = require('${module}')}catch(err){console.warn(err.message)}return r})()`
          code = replaceString(code, replace, safe)
          if (es2015) {
            es2015 = replaceString(es2015, replace, safe)
          }
        } else {
          if (!bundle.errors) {
            bundle.errors = {}
        ...
        if (type === 'import' || type === 'require') {
          ...
          members.forEach(member => {
            ...
            if (
              ...
              !(
                member === 'default' &&
                code.indexOf('__TREESHAKE_DEFAULT__') !== -1
              )
            ) {
              code =
                member === 'default'
                  ? `\nvar ${variableName} = (${target.id}.${member} !== void 0 ? ${target.id}.${member} : ${target.id});\n` +
                    code
          ...
          if (defaultExports) {
            ...
            if (parsed.exportStats.moduleExports.includes('__esModule')) {
              ...
              code = replaceString(
                ...
                `(${target.id}.default !== void 0 ? ${target.id}.default : ${target.id})`
              )
              if (es2015) {
                es2015 = replaceString(
                  es2015,
  ...
  bundle.js.push(
    `// ================== ${file.resolved[bundleType]} ${file.id} ===================`
  )
  if (es2015) {
    bundle.jsEs2015.push(
      `// ================== ${file.resolved[bundleType]} ${file.id} ==========ES2015=========`
    )
  ...
  if (!parsedFile.exportStats.module && !parsedFile.exportStats.default) {
    if (
      ...
      if (code.indexOf('__TREESHAKE_DEFAULT__') === -1) {
        const def = `${file.id}_default`
        bundle.vars.push(`var ${file.id} = {};`)
        bundle.js.push(`${file.id} = ${def}`)
        if (es2015) {
          bundle.jsEs2015.push(`${file.id} = ${def}`)
        }
      } else {
  ...
  if (parsedFile.env.length) {
    parsedFile.env.forEach(v => {
      bundle.env.add(v)
    })
  }
