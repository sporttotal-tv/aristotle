...
const buildResult = async props => {
  ...
  if (type !== 'node') {
    if (browser.css) {
      if (browser.styles) {
        for (const key in browser.styles) {
          if (key.indexOf('style') === 0) {
            let id = stats.style.parsed[key] || uid(stats.style.cnt++)
            // these classnames are used for reactnative web
            if (id === 't' || id === 'v' || id === 'i') {
              id = uid(stats.style.cnt++)
            }
    ...
    if (props.minify !== false) {
      ...
      browser.minEs2015 = browser.es2015
      node.min = node.code
    }
    ...
    if (
      ...
      for (const key in browser.bundleMap) {
        await buildResult({
          ...props,
          bundle: {
            parsed: true,
