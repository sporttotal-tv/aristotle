...
const production = async ({
  ...
  if (result.ssl) {
    const files = await fs.readdir(result.ssl)

    const crt = files.find(v => /\.crt$/.test(v))

    const key = files.find(v => /\.key$/.test(v))
    if (key && crt) {
      files.crt = {
        contents: await fs.readFile(join(result.ssl, crt))
      }
      files.key = {
        contents: await fs.readFile(join(result.ssl, key))
      }
    } else {
      console.error('please add a .crt and .key file for ssl support')
    }
  }
