...
const TMPDIR = '/var/tmp/aristotle'
...
const generateName = file => {
  ...
  return chalk.white(process.env.NODE_ENV || 'development') + ' ' + id
}
  ...
  if (key && store.cache[key] && file.pkg._id) {
    try {
      ...
      const cached = JSON.parse(
        (await fs.readFile(join(TMPDIR, key))).toString()
      )
      if (cached) {
        return addToFile(file, cached)
      }
    } catch (err) {
      console.log(chalk.red(`• Failed to get from cache ${file.path}`))
    }
  }
}
