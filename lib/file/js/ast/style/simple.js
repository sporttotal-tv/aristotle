...
const simpleStyle = (
  ...
  if (
    val.key.name === 'backgroundColor' ||
    val.key.name === 'background' ||
    val.key.name === 'borderColor' ||
    val.key.name === 'border'
  ) {
    const name = dash(val.key.name)
    // collectProperties
    const value = `${name}: \${${generate(val.value).code}}`

    dynamicStyles.push(`__dynC(\`${value}\`, 'c')`)
    return true
  }
}
