...
// allways wait until you receive code for a process

// process.on('message', m => {
//   const result = {}
//   for (const key in m) {
//     const ast = exports[key](...m[key])
//     result[key] = ast
//   }
//   process.send(result)
// })
...
exports.parseCode = (file, raw, dontParseRequires, type, store) => {
  // id, path, resolved
  ...
  try {
    ...
    var { ast: ast3 } = babel.transformFromAst(ast, raw, {
      ...
      plugins: [
        ...
        webpackInterop({ exportStats, id, idMap })
      ]
    })

    var { code } = babel.transformFromAst(ast3, raw, {
      plugins: [webpackInterop({ exportStats, id, idMap })]
    })
