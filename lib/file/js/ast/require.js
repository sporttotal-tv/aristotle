...
const isLiteral = prop => {
  return prop.type === 'StringLiteral' || props.type === 'NumberLiteral'
}
          ...
          if (
            ...
            for (let i = 0; i < length; i++) {
              ...
              if ((!prop.computed || isLiteral(prop)) && !prop.shorthand) {
                if (prop.key.name === 'default' || prop.key.value === 'default') {
                  hasDefault = prop
                } else if (prop.key.name === '__esModule' || prop.key.value === '__esModule') {
                  hasEsmodule = true
                  exportStats.__esModule = true
                }
              }
