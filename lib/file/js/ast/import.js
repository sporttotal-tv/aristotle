...
const excludeNodeModule = require('../../../excludeNodeModules')

const parseImportDeclaration = (node, imports, path, store, nodeModules) => {
  const moduleName = node.source.value

  if (excludeNodeModule(moduleName, store)) {
  ...
  node.specifiers.forEach(node => {
    ...
    if (node.type === 'ImportNamespaceSpecifier') {
      ...
      // if (moduleName === 'react-native' && importedName === 'View') {
...
const parseImports = ({ exportStats, id, imports, store, nodeModules }) => {
  return {
        ...
        node.specifiers.forEach(node => {
          if (node.type === 'ExportSpecifier') {
            ...
            if (node.local.name === 'default') {
              ...
              importer.members.push(node.local.name)
              if (node.exported.name === 'default') {
                exportStats.default = true
                NAME = t.identifier(id)
                SOURCE = t.identifier(
                  rewriteModule(moduleName, node.local.name)
                )
              } else {
                exportStats.export.push(node.exported.name)
                NAME = t.identifier(id + '_' + node.exported.name)
            ...
            path.insertBefore(variable({ NAME, SOURCE }))
          }
        })
        path.remove()
      } else {
    ...
    ImportDeclaration(path) {
      parseImportDeclaration(path.node, imports, path, store, nodeModules)
    }
  }
}
