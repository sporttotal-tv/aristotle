...
const es2015 = code =>
  new Promise((resolve, reject) => {
    ...
    const args = [code]
    child.send({ es2015: args })
  })

const parseJs = async (file, fromCache) => {
  file.type = 'js'
  ...
  try {
    ...
    if (
      ...
      file.node.parsed = await parse(file, 'node', fromCache)
      if (!fromCache) {
      ...
      file.browser.parsed = await parse(file, 'browser', fromCache)
    }

    if (!fromCache) {
      file.browser.parsed.es2015 = await es2015(file.browser.parsed.code)
    }
    ...
    file.error = new Error('Error parsing AST of file: ' + err.message)
  }
}
