...
if (!resolveStylesheet) {
  ...
  for (let i = 0; i < styleSheets.length; i++) {
    if (styleSheets[i] && styleSheets[i].rules) {
      for (let j = 0; j < styleSheets[i].rules.length; j++) {
        const rule = styleSheets[i].rules[j]
        const s = rule.selectorText
        if (s && s[s.length - 1] === '_') {
          resolveStylesheet = styleSheets[i]
          break
        }
      }
    }
...
if (resolveStylesheet) {
  for (let i = 0; i < resolveStylesheet.rules.length; i++) {
    const rule = resolveStylesheet.rules[i]
    const s = rule.selectorText
