...
class Item extends React.Component {
  shouldComponentUpdate(next) {
    const change =
      next.item !== this.props.item ||
      (next.item && this.props.item && next.item.type !== this.props.item.type)
    return change
  }
  render() {
      ...
      this.timer = global.requestAnimationFrame(() => {
        if (this.element) {
          ...
          if (
            ...
            const amount = this.state.amount + 30
            this.lastSet = amount
  ...
  shouldComponentUpdate(next, state) {
    ...
    if (next.sections !== this.props.sections) {
      ...
      if (
        ...
        next.sections[0].checksum &&
        this.props.sections[0].checksum &&
        // eslint-disable-next-line
        next.sections[0].checksum.split('-')[0] ==
          this.props.sections[0].checksum.split('-')[0]
      ) {
        samePage = true
          ...
          <div
            ...
            style={{
              ...
              height:
                !this.currentAmount ||
                !this.size ||
                this.currentAmount < this.size - 1
                  ? 2000
                  : 0
            }}
          />
