...
module.exports = async (store, port, path) => {
  ...
  const createFile = (file, env) => {
    const m = /\.es5$/.test(file.path)
      ? 'application/javascript'
      : mime.lookup(file.path)

    if (m === 'application/javascript' && env) {
      const obj = JSON.parse(env.contents)
  ...
  for (const key in files) {
    pathMap[files[key].path] = createFile(files[key], files.env)
  }
  ...
  const reportServerError = (err, init, errors, code = '') => {
    ...
    pathMap[path] = createFile(files.js, files.env)
  }
    ...
    if (server && !Object.keys(error).length) {
      if (lastServer !== server.codeHash + (files.css ? files.css.path : '')) {
        ...
        try {
          ...
          child.send(
            JSON.stringify({
              requireContext: server.requireContext,
              code: ssrCode
            })
          )
  ...
  http
    .createServer(async (req, res) => {
      ...
      if (mappedFile) {
        ...
        let file = join(pkgDir, req.url)

        fs.stat(file, async (err, data) => {
          if (err) {
            file = join(pkgDir, 'public', req.url)
            fs.stat(file, async (err, data) => {
              if (err) {
