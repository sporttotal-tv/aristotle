        ...
        process.stdin.on('readable', async () => {
          ...
          if (/__\\$\\$\\$END__$/.test(data)) {
            ...
            try {
              ...
              try {
                const r = await ${server.id}(...args.args);
                process.stdout.write(startIndicator+ JSON.stringify({
                  seq: args.seq,
                  content: r
                }) + endIndicator);
              } catch (err) {
      ...
      if (lastServer !== server.codeHash) {
        ...
        try {
          ...
          child.stdout.on('data', data => {
            const isStart = data.toString().indexOf(startIndicator) === 0
            if (isReceiving || isStart) {
              isReceiving = true
              if (isStart) {
                chunkedData += data.toString().slice(startIndicator.length)
              } else {
                chunkedData += data.toString()
              }
