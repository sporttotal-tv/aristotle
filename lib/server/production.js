...
const initServer = async port => {
  ...
  if (isSSL) {
    try {
      ...
      https.createServer(options, serverRespond).listen(port)
    } catch (err) {
      console.error(chalk.red('Invalid SSL certificate'), err.message)
      http.createServer(serverRespond).listen(port)
    }
  } else {
    http.createServer(serverRespond).listen(port)
  }
  ...
  console.log(
    chalk.blue(
      `🛸  Start production ${
        isSSL ? 'SSL' : ''
      } server on port "${port}" maxCache ${cacheTime} seconds`
    )
  )
}
...
const port = (process.env.PORT || process.argv[3] || 8082) * 1
